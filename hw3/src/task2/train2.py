...
def get_model(train_graph_path , out_model_path):
    ...
    print("Loading Data...")
    # -------------------- Data Loading --------------------
    user_feats    = np.load(f'{train_graph_path}/user_features.npy')    # (m, d1)
    product_feats = np.load(f'{train_graph_path}/product_features.npy') # (p, d2)
    edges         = np.load(f'{train_graph_path}/user_product.npy').astype(int)  # (e,2)
    labels        = np.load(f'{train_graph_path}/label.npy')           # (m, ℓ)
    ...
    Y   = torch.from_numpy(labels).float().to(device)        # (m, ℓ)
    ...
    all_users = np.arange(m)
    # train_idx = torch.tensor(all_users, dtype=torch.long, device=device)
    train_idx, val_idx = train_test_split(all_users, test_size=0.2, random_state=42)
    train_idx = torch.tensor(all_users, dtype=torch.long, device=device)
    ...
    # class imbalance
    y_pos = Y[train_idx].sum(dim=0)
    y_neg = train_idx.size(0) - y_pos
    pos_weight = (y_neg / (y_pos + 1e-6)).to(device)
    ...
    for epoch in range(1, 501):
        ...
        model.train()
        optimizer.zero_grad()
        logits = model(X_u, X_p, users_idx, prods_idx, norm)         # (m,ℓ)
        loss   = criterion(logits[train_idx], Y[train_idx])          # scalar
        loss.backward()
        optimizer.step()
        ...
        if epoch % 10 == 0 :
            model.eval()
            with torch.no_grad():
                val_logits = model(X_u, X_p, users_idx, prods_idx, norm)
                val_loss   = criterion(val_logits[val_idx], Y[val_idx]).item()

                probs = torch.sigmoid(val_logits[val_idx]).cpu().numpy()
