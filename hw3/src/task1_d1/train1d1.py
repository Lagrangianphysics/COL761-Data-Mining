...
def get_model(train_graph_path , out_model_path):
    ...
    for idx, (model, opt, sch) in enumerate(zip(models, optimizers, schedulers), 1):
        ...
        for epoch in range(1, epochs+1):
            ...
            model.eval()
            with torch.no_grad():
                probs = out[val_idx].exp().cpu().numpy()
                true  = data.y[val_idx].long().cpu().numpy()
                auc   = roc_auc_score(np.eye(output_dim)[true], probs, multi_class='ovo')
            ...
            if auc > best_auc:
                ...
                wait += 1
                if wait >= patience:
                    print(f"  • Model {idx}: early stop @ epoch {epoch}, best AUC={best_auc:.4f}")
                    break

            if epoch % 10 == 0:
                print(f"  • Model {idx} | Epoch {epoch} | Loss {loss:.4f} | Val AUC {auc:.4f}")
    ...
    true      = data.y[val_idx].long().cpu().numpy()
    auc_final = roc_auc_score(np.eye(output_dim)[true], val_probs, multi_class='ovo')
    print(f"Ensemble Validation AUC: {auc_final:.4f}")
